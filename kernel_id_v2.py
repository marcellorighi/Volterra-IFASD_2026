...
pitch_input = 1e0*gen_disp[1,::10]*29.85872 /180*np.pi # pitch input if you want to convert to deg
...
plt.bar(np.arange(m*3),theta_cl2)
plt.xlabel(r'$\tau\,[\,]$')
plt.ylabel(r'$w\, [\,]$')
plt.legend()
plt.grid(True)
...
plt.bar(np.arange(m*3),theta_cm2)
plt.xlabel(r'$\tau\,[\,]$')
plt.ylabel(r'$w\, [\,]$')
plt.legend()
plt.grid(True)
...
heave_input_int = 1e0*gen_disp[0,::10]*0.106654261 # heave input if you want to convert to m
pitch_input_test = 1e0*gen_disp[1,::10]*29.85872 /180*np.pi # pitch input if you want to convert to deg

heave_input_test = 1.0*heave_input_int #np.gradient(heave_input_int,time_vector)
...
cl_output_test = gen_force[0,::10]
cm_output_test = gen_force[1,::10]
