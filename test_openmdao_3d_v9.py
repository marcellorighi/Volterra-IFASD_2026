...
def run_volterra_identification(A, cl, cm, m, 
    ...
    if use_laguerre:
        ...
        prob.model.add_design_var('c_cl', lower=-4500.0, upper=4500.0)
        prob.model.add_design_var('c_cm', lower=-4500.0, upper=4500.0)
    else:
...
# fourth order
A_pitch4 = A_pitch*A_pitch*A_pitch*A_pitch
A_zdot4 = A_zdot*A_zdot*A_zdot*A_zdot
A_pitch2_zdot2 = A_pitch*A_pitch*A_zdot*A_zdot
A_pitch_zdot3 = A_pitch*A_zdot3
A_pitch3_zdot = A_zdot*A_pitch3

A4 = np.hstack( (A_pitch4,A_zdot4,A_pitch2_zdot2, A_pitch3_zdot,A_pitch_zdot3))

# TEMP
cl_norm_factor = 4. #4. #5. #5.
cm_norm_factor = 30. #30. # 20. 

delta_cl_norm_factor = 1. #30. 
delta_cm_norm_factor = 1. #30. 

cl_output *= cl_norm_factor
cm_output *= cm_norm_factor 

# A_123 = np.hstack( (A, A2, A3) )
A_123 = np.hstack( (A, A2, A3, A4) )

# Example with your data
h_cl, h_cm, prob = run_volterra_identification(
    A=A_123,           # Your A matrix
    cl=cl_output,         # Your cl vector
    cm=cm_output,         # Your cm vector
    m=m,          # Your memory length
    use_laguerre=True,  # or True for basis expansion
    n_basis=16,  # Fewer parameters than direct (8 vs 20 per order)
    alpha_laguerre=0.36,
    lambda_smooth=0.000,
    lambda_decay=0.0625, # 0.1
...
# Access results
print(f"Identified h_cl shape: {h_cl.shape}")
print(f"Identified h_cm shape: {h_cm.shape}")

cl_check = A_123@h_cl / cl_norm_factor
cm_check = A_123@h_cm / cm_norm_factor
...
plt.subplot(1, 2, 1)
plt.plot(h_cl_1, 'r-', linewidth=1,label="1st order")
plt.plot(h_cl_2, 'b-', linewidth=1,label="2nd order")
plt.plot(h_cl_3, 'g-', linewidth=1,label="3rd order")
plt.plot(h_cl_4, 'k-', linewidth=1,label="4th order")
plt.xlabel('Lag index')
plt.ylabel('Coefficient')
plt.legend()
plt.title(f'h_cl - Order {order}')
...
plt.subplot(1, 2, 2)
plt.plot(h_cm_1, 'r-', linewidth=1,label="1st order")
plt.plot(h_cm_2, 'b-', linewidth=1,label="2nd order")
plt.plot(h_cm_3, 'g-', linewidth=1, label="3rd order")
plt.plot(h_cm_4, 'k-', linewidth=1, label="4th order")
plt.xlabel('Lag index')
plt.ylabel('Coefficient')
plt.title(f'h_cm - Order {order}')
plt.legend()
...
# save kernels 
filename = home_pitch + "/kernels_1_to_4.npz"    
np.savez(
    filename, 
    m=m, 
    cl_0= cl_0, 
    ...
    h_cl_2=h_cl_2 * (norm_factor**2) / cl_norm_factor, 
    h_cm_2=h_cm_2 * (norm_factor**2) / cm_norm_factor,
    h_cl_3=h_cl_3 * (norm_factor**3) / cl_norm_factor, 
    h_cm_3=h_cm_3 * (norm_factor**3) / cm_norm_factor,
    h_cl_4=h_cl_4 * (norm_factor**4) / cl_norm_factor, 
    h_cm_4=h_cm_4 * (norm_factor**4) / cm_norm_factor
)
...
A_123_test = np.hstack( (A, A2, A3, A4))

h_123 = np.hstack( (h_cl_1 * norm_factor / cl_norm_factor, h_cl_2* (norm_factor**2) / cl_norm_factor, h_cl_3* (norm_factor**3) / cl_norm_factor, h_cl_4* (norm_factor**4) / cl_norm_factor) )
# h_123 = np.hstack( (h_cl_1 * norm_factor / cl_norm_factor, h_cl_2* (norm_factor**2) / cl_norm_factor, h_cl_3* (norm_factor**3) / cl_norm_factor) )
cl_test = A_123_test@h_123

h_123_cm = np.hstack( (h_cm_1 * norm_factor / cm_norm_factor, h_cm_2* (norm_factor**2) / cm_norm_factor, h_cm_3* (norm_factor**3) / cm_norm_factor, h_cm_4* (norm_factor**4) / cm_norm_factor) )
# h_123_cm = np.hstack( (h_cm_1 * norm_factor / cm_norm_factor, h_cm_2* (norm_factor**2) / cm_norm_factor, h_cm_3* (norm_factor**3) / cm_norm_factor) )
...
plt.subplot(1, 2, 1)
plt.plot(cl_output_test, 'b-', linewidth=2, label="CFD")
plt.plot(cl_test, 'k-', linewidth=1, label="V4")
plt.xlabel(f't')
plt.ylabel(f'C_l')
plt.grid(True)
plt.legend()
...
plt.subplot(1, 2, 2)
plt.plot(cm_output_test, 'b-', linewidth=2, label="CFD")
plt.plot(cm_test, 'k-', linewidth=1, label="V4")
plt.xlabel(f't')
plt.ylabel(f'C_m')
plt.grid(True)
plt.legend()
