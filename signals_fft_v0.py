...
amp1 = 2*np.random.rand(Nk) *np.pi/180.
aoa1 = np.zeros_like(time_vector)

for k in range(Nk): 
    phi[k] = np.pi/Nk*k*(k-1)
    aoa1 += amp1[k] * np.cos( (k+1)*kk1*time_vector + phi[k])
...
amplitude = 2.0/ntsteps * np.abs(fft_values[positive_mask])
...
# Time Domain
ax1.plot(time_vector, pitch_input, color='#1f77b4', lw=1)
...
# Frequency Domain
ax2.stem(freqs, amplitude, basefmt=" ", linefmt='r-', markerfmt='ro')
